...
import com.seckill.miaosha.access.AccessLimit;
...
public class MiaoshaController implements InitializingBean {
    ...
    public Result<String> getPath(HttpServletRequest request,MiaoshaUser user, Model model,
        ...
        /**接口防刷： 用一个计数器来控制 这个是针对 每个url ，所以缓存中的值 key 必须要通过 url来区分，而不是用户id和商品号，因为可能用户点不同的url 也给限流
         * */
...
//        }
        ...
        int verifyCodeI;
        try {
             verifyCodeI= Integer.valueOf(verifyCode);
        }catch (Exception e){
            return Result.error(CodeMsg.MIAO_SHA_VERICODE);
        }
        boolean eval = miaoshaService.checkVerifyCode(user,goodsId,verifyCodeI);
        if (!eval)
        {
